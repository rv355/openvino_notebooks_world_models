...
bowl and place it on the plate"*, it predicts the end-effector motion that carries the task out —
...
* **A DiT flow-matching action head** (150M parameters) that conditions on those 32 tokens plus the
  robot's proprioceptive state and denoises a noise sample into a 7-step action chunk over 4 Euler
...
- Pre-trained VLA-JEPA weights under `pretrained/`, and the fixed sample observation in `golden_outputs/`
